...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| 状态检查周期 | 45 秒 | 多久问一次 portal“我还在线吗”，**最小 5 秒** |
| 空闲阈值 | 150 秒 | 连续多少秒没流量才补发保活流量 |
| 保活周期 | 300 秒 | 读不到网卡统计时的兜底补流量周期 |
| 定时重连 | 留空 | 每天到该时刻先注销再重新认证，例如 `02:00`；**留空则不启用** |
| 睡眠唤醒后立即重连 | 开启 | 从睡眠/休眠恢复后立刻重新认证一次 |

### 手动断开不会自动重连

点“断开”后程序会进入“手动断开”状态，**不再自动重连**，也不会因为探活失败
而重新登录；这个状态会写进配置（`want_online=0`），所以关掉程序再打开也不会
自己连上。要恢复自动重连，点一次“连接”即可（或等定时/唤醒事件触发）。

### 定时重连与唤醒重连

* 两者都会**先注销再登录**，目的是拿到全新会话，而不是复用旧的。
* 定时重连按“当前时间的 HH:MM 是否等于设定值”判断，同一天只执行一次。
* 唤醒检测靠主循环的时间跳变：循环每 5 秒一轮，若一次等待后发现墙上时钟
  跳了 120 秒以上，即判定为睡眠归来。
* 这两类事件都会把 `want_online` 置回 1（即重新进入自动重连状态）。
